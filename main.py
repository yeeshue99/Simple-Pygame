...
for _ in range(enemy_count):
    pos_x = random.randint(50, width)
    pos_y = random.randint(50, height)
...
# Setting how fast the player will move (// means integer division)
moveSpeed = 60 // 10
...
while True:
    ...
    if move_right:
        if(circle_pos[0] + moveSpeed < width):
            circle_pos = (circle_pos[0] + moveSpeed, circle_pos[1])
        else:
            circle_pos = (0, circle_pos[1])
    if move_left:
        if(circle_pos[0] + moveSpeed > 0):
            circle_pos = (circle_pos[0] - moveSpeed, circle_pos[1])
        else:
            circle_pos = (width, circle_pos[1])
    if move_down:
        if(circle_pos[1] - moveSpeed < height):
            circle_pos = (circle_pos[0], circle_pos[1] + moveSpeed)
        else:
            circle_pos = (circle_pos[0], 0)
    if move_up:
        if(circle_pos[1] + moveSpeed > 0):
            circle_pos = (circle_pos[0], circle_pos[1] - moveSpeed)
        else:
            circle_pos = (circle_pos[0], height)
